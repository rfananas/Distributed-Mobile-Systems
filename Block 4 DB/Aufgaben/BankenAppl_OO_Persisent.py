...
class Konto:
    ...
    def __str__(self):
        return "id: {id:6d}    saldo: {saldo:10.2f}    limite: {limite:10.2f}    kontoArt: {kArt:20s}    " \
               "owner: {owner:30s}".format(id=self.__id, saldo=self.__saldo, limite=self.__limite,
                                           kArt=self.__kontoArt, owner=self.__owner)
    ...
    def setSaldo(self, newSaldo, doCommit = True):
        ...
        if self.__cursor is not None:
            sql_update_query = """UPDATE MyBank.bankkonto set saldo = """ + str(newSaldo) + """ where id_bankkonto = """ + str(self.__id)
    ...
    def bezug(self, amount, doCommit = True):
        ...
        if self.__cursor is not None:
            sql_update_query = """Update MyBank.bankkonto set saldo = saldo - """ + str(effBezug) + """ where id_bankkonto = """ + str(self.__id)
            self.__cursor.execute(sql_update_query)
            if doCommit:
                self.__conn.commit()
        return effBezug
    ...
    def deposit(self, amount, doCommit = True):
        ...
        if self.__cursor is not None:
            sql_update_query = """Update MyBank.bankkonto set saldo = saldo + """ + str(amount) + """ where id_bankkonto = """ + str(self.__id)
            self.__cursor.execute(sql_update_query)
            if doCommit:
                self.__conn.commit()
...
# ----------------------------------------------------------------------
class Kontoliste:
    def __init__(self, owner="Unknown", dbServer = "localhost", dbSchema = "MyBank", userName = "root", password = "admin", auth_plugin='mysql_native_password'):
        self.__owner = owner
        self.__kontoList = {}
...
def Test_Kontoliste():
    ...
    # Initialisierung Kontoliste, Übergabe Name vom Kontoinhaber
